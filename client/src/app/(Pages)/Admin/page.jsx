...
function Page() {
  ...
  const handleDelete = async (formNo) => {
    ...
    try {
      const response = await fetch(
        `https://forms-backend-hanl.onrender.com/api/v1/forms/delete/${formNo}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (response.ok) {
        ...
        setFormData((prevFormData) =>
          prevFormData.filter((form) => form.form_no !== formNo)
        );
            ...
            formData.map((form) => (
              <tr key={form.form_no}>
                ...
                <td>
                  <button
                    className="edit-btn"
                    onClick={() => handleEdit(form.form_no)}
                  >
                    Edit
                  </button>
                  <button
                    className="delete-btn"
                    onClick={() => handleDelete(form.form_no)}
                  >
                    Delete
                  </button>
