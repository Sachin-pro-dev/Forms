...
function EditForm() {
  ...
  // Fetch the form details for the form number
  useEffect(() => {
    if (form_no) {
      const fetchFormDetails = async () => {
        try {
          const response = await fetch(`http://localhost:5500/api/v1/forms/read/${form_no}`);
          if (!response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:5500/api/v1/forms/update/${form_no}`, {
        method: "PUT",
        headers: {
